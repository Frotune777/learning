    ...
    for position in positions:
        try:
            ...
            if not br_symbol or br_symbol is None or str(br_symbol).lower() == "none":
                logger.warning(
                    f"Symbol not found for: {symbol} on exchange: {exchange}"
                )
            ...
            if not br_symbol_str:
                ...
                skipped_positions.append(
                    f"{symbol} ({exchange}) - invalid symbol: {br_symbol_str}"
                )
                continue
            ...
            skipped_positions.append(
                f"{position.get('symbol', 'unknown')} - Error: {e!s}"
            )
            continue
...
def parse_margin_response(response_data):
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Failed to parse margin response: {e!s}",
        }
