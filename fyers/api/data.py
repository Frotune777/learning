...
def get_api_response(endpoint, auth, method="GET", payload=""):
    ...
    try:
        ...
        logger.error(f"HTTP error during API request: {e!s}")
        return {"s": "error", "message": f"HTTP error: {e!s}"}
    except json.JSONDecodeError as e:
        logger.error(f"JSON decode error: {e!s}")
        return {"s": "error", "message": f"Invalid JSON response: {e!s}"}
    except Exception as e:
        logger.exception("An unexpected error occurred during API request")
        return {"s": "error", "message": f"General error: {e!s}"}
...
class BrokerData:
    ...
    def get_history(
        ...
        try:
            ...
            if resolution == "1D":
                ...
                chunk_days = (
                    25  # For seconds data - max 30 trading days, use 25 to be safe
                )
            else:
    ...
    def get_depth(self, symbol: str, exchange: str) -> dict:
        ...
        try:
            ...
            logger.debug(
                f"Fyers depth API FULL response: {json.dumps(response, indent=2)}"
            )
            ...
            asks = depth_data.get(
                "ask", []
            )  # Note: Fyers uses 'ask' (singular) not 'asks'
            ...
            bids_formatted = [
                {"price": b.get("price", 0), "quantity": b.get("volume", 0)}
                for b in bids[:5]
            ]
            asks_formatted = [
                {"price": a.get("price", 0), "quantity": a.get("volume", 0)}
                for a in asks[:5]
            ]
