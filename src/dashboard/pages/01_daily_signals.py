...
Purpose: Daily trading signals — multi-factor scored & ranked opportunities.
         Upgraded to screeni-py-level GUI: color-coded consensus badges,
         strategy heatmap, inline TA scorecard, optional export.
...
from src.portfolio.portfolio_engine import Portfolio, PortfolioEngine
...
enriched_files = sorted(glob.glob(os.path.join(processed_dir, "top_250_enriched_*.csv")))
target_files   = analyzed_files if analyzed_files else enriched_files

if not target_files:
    st.error("❌ No screening output found. Run the Technical Screener (CLI option 4) first.")
    st.stop()
...
strategy_options = [
    "All", "In Bull Run", "Start GTT",
    "STR_NIFTY_SHOP", "STR_BUY_LOW", "STR_TURTLE", "STR_RDX",
    "STR_100SMA", "STR_ETF_SHOP", "STR_SUPER_BO",
    "STR_VCP",    "STR_TTM",    "STR_SUPERTREND",    "STR_LORENTZIAN",
]
selected_strat = st.sidebar.selectbox("Filter by Setup", strategy_options)

min_consensus = st.sidebar.slider("Min Consensus Score", min_value=0, max_value=13, value=0)
...
if selected_strat != "All":
    if selected_strat == "In Bull Run":
        ...
        filtered = filtered[filtered["BOTTOM_OUT_STATUS"].isin(["Start GTT","Start GTT (Basic)"])]
    else:
...
if min_consensus > 0 and "CONSENSUS_SCORE" in filtered.columns:
    filtered = filtered[pd.to_numeric(filtered["CONSENSUS_SCORE"], errors="coerce").fillna(0) >= min_consensus]
...
# KPI Cards
# ---------------------------------------------------------------------------
bull_count     = int((df["TREND_STATUS"] == "In Bull Run").sum()) if "TREND_STATUS" in df.columns else 0
high_conv      = int((pd.to_numeric(df.get("CONSENSUS_SCORE", pd.Series()), errors="coerce") >= 4).sum())
gtt_count      = int(df["BOTTOM_OUT_STATUS"].isin(["Start GTT","Start GTT (Basic)"]).sum()) if "BOTTOM_OUT_STATUS" in df.columns else 0
new_strat_hits = 0
for col in ["STR_VCP_ACTION","STR_TTM_ACTION","STR_SUPERTREND_ACTION","STR_LORENTZIAN_ACTION"]:
...
DISPLAY_COLS_PRIORITY = [
    "SYMBOL", "CMP", "TREND_STATUS", "CAR_RATING", "CONSENSUS_SCORE",
    "CONSENSUS_CALLOUT", "TECH_SCORE", "RSI_14", "ATR_14", "DELIV_PCT",
    "STR_VCP_ACTION", "STR_TTM_ACTION", "STR_SUPERTREND_ACTION", "STR_LORENTZIAN_ACTION",
    "RSI_DIVERGENCE", "MACD_DIVERGENCE", "STR_CANDLE_PATTERN",
    "Corp Action", "Insider Score",
]
...
if not filtered.empty:
    ...
    styled = (
        ...
        .style
        .applymap(_color_consensus, subset=["CONSENSUS_SCORE"] if "CONSENSUS_SCORE" in display_cols else [])
        .applymap(_color_trend, subset=["TREND_STATUS"] if "TREND_STATUS" in display_cols else [])
        .format({
            "CMP": "₹{:,.2f}",
            "RSI_14": "{:.1f}",
            "ATR_14": "{:.2f}",
...
with st.expander("🔥 Strategy Signals Heatmap", expanded=True):
    strat_action_cols = [c for c in df.columns if c.startswith("STR_") and c.endswith("_ACTION")]
    if strat_action_cols and not filtered.empty:
        ...
        heat_num.columns = [c.replace("STR_","").replace("_ACTION","").replace("_"," ").title() for c in heat_num.columns]
        st.dataframe(
...
if not filtered.empty:
    ...
    with col3:
        ...
        if st.button("Place Simulated BUY"):
            ...
            if port.add_position(buy_sym, qty, entry_price, stop_price, target_price, sector=sector_name):
