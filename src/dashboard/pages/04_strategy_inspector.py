...
Purpose: Strategy Inspector & Consensus Leaderboard — view all 13 strategy
         signals per stock, consensus rankings, and narratives.
         Modelled on screeni-py's detailed stock view (Format 5 + Format 6).
...
analyzed_files = sorted(glob.glob(os.path.join(processed_dir, "top_250_analyzed_*.csv")))

if not analyzed_files:
    st.error("❌ No analyzed data found. Run the Technical Screener (CLI option 4) first.")
    st.stop()
...
df = pd.read_csv(next(f for f in analyzed_files if os.path.basename(f) == selected_file))

# Add/recalculate consensus & decision support if missing
if "CONSENSUS_SCORE" not in df.columns or "WEIGHTED_BULL_SCORE" not in df.columns or "MARKET_STATE" not in df.columns:
    try:
...
with tab_lead:
    ...
    ranked = df.copy()
    ranked["CONSENSUS_SCORE"] = pd.to_numeric(ranked.get("CONSENSUS_SCORE", 0), errors="coerce").fillna(0)
    ranked = ranked[ranked["CONSENSUS_SCORE"] >= min_score].sort_values("CONSENSUS_SCORE", ascending=False)

    if ranked.empty:
        st.info("No stocks meet the minimum consensus score.")
    else:
        # Build display table
        strat_cols_present = [col for col in STRATEGY_MAP.values() if col in ranked.columns]
        disp = ranked[["SYMBOL"] + strat_cols_present + [
            c for c in [
                "CONSENSUS_SCORE", "WEIGHTED_BULL_SCORE", "WEIGHTED_BEAR_SCORE",
                "MARKET_STATE", "PORTFOLIO_ACTION", "CONFIDENCE_PCT",
                "CMP", "TREND_STATUS", "RSI_14", "TECH_SCORE"
        ...
        styled = disp.style.applymap(
            ...
            lambda v: "background-color:#00c27c20;color:#00c27c;font-weight:700" if isinstance(v, (int,float)) and v >= 4
            else "color:#7c83fd;font-weight:600" if isinstance(v, (int,float)) and v > 0
            else "color:#888",
...
with tab_insp:
    ...
    row = df[df["SYMBOL"] == symbol].iloc[0] if not df[df["SYMBOL"] == symbol].empty else None
    if row is None:
    ...
    dq_color = "#00c27c" if dq_stat == "HEALTHY" else "#ffaa00" if dq_stat in ("LOW_COVERAGE", "ZERO_VOLUME", "ZERO_CLOSE") else "#ff5555"
    ...
    for col, icon, label, val in [
        ...
        card_val_size = "1.02rem" if label == "Data Quality" and len(val) > 12 else "1.4rem"
        col.markdown(f"""
    ...
    target_str = f"₹{target_20pct:,.2f}" if pd.notna(target_20pct) and target_20pct > 0 else "—"
    qty_str = str(int(suggested_qty)) if pd.notna(suggested_qty) and suggested_qty > 0 else "—"
    
    # Dynamic colors for action
    action_color = "#00c27c" if "BUY" in p_action else "#ff5555" if "EXIT" in p_action or "REDUCE" in p_action else "#888"
    state_icon = "📈" if m_state == "BULL RUN" else "🔄" if m_state == "RECOVERY" else "📉" if m_state == "BEAR TERRITORY" else "↕"
    ...
    for name, col_key in STRATEGY_MAP.items():
        ...
        if action_val in _BUY_VALS:
            ...
            kind  = "HOLD"
        strat_rows.append({"Strategy": name, "Action": action_val, "Signal": kind, "_badge": badge})
    ...
    def _mk_html_table(df_s):
        ...
        for _, r in df_s.iterrows():
            cls = "signal-buy" if r["Signal"]=="BUY" else "signal-sell" if r["Signal"]=="SELL" else "signal-hold"
    ...
    with d3:
        ...
        candle_badge = f'<span class="badge-buy">{candle}</span>' if candle not in ("None","N/A","") else '<span class="badge-hold">None</span>'
    ...
    if callout and callout != "N/A":
        color = "#00c27c" if "BUY" in callout else "#ff5555" if "SELL" in callout else "#888"
        st.markdown(f"""
