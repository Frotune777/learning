...
Purpose: Render backtest configurations and results for trading strategies.
...
Dependencies:
    ...
    Internal:
        - src.nse_bhavcopy.ta_indicators: Compute technical indicators
        - src.nse_bhavcopy.backtester: Vectorized and event-driven backtesting engines
...
Open Tasks:
    - [ ] [MEDIUM] Add backtesting for new strategy engines
...
from src.nse_bhavcopy.backtester import NSEEventBacktester, VectorBTBacktester
...
if not parquet_files:
    st.error(
        "No historical Parquet files found. Run sync-history or bhavcopy-sync in the CLI first."
    )
else:
    ...
    init_cash = st.sidebar.number_input(
        "Initial Capital (INR)", min_value=1000.0, value=100000.0, step=10000.0
    )
    ...
    if strategy == "Turtle Trading (55D Breakout)":
        ...
        for idx, row in df.iterrows():
            if (
                position == 0
                and adx.loc[idx] > 25.0
                and plus_di.loc[idx] > minus_di.loc[idx]
                and rsi.loc[idx] > 60.0
            ):
                position = 1
                entries.loc[idx] = True
            elif position == 1 and (
                plus_di.loc[idx] < minus_di.loc[idx] or rsi.loc[idx] < 45.0
            ):
                position = 0
                exits.loc[idx] = True
            signals.loc[idx] = position
        ...
        for idx, row in df.iterrows():
            ...
            if (
                position == 0
                and close > sma_200.loc[idx]
                and sma_200.loc[idx] > sma_150.loc[idx]
            ):
                position = 1
                entries.loc[idx] = True
    ...
    with col_bt2:
        ...
        try:
            res_vbt = VectorBTBacktester.run_backtest(
